...
with torch.no_grad():
    ...
    for batch_id, data in enumerate(test_dataloader):
        ...
        #     head_array = np.frombuffer(f.read(4*2), dtype=np.int16)        
        ...
        with open(trans_binname, 'rb') as f:
            trans_array = np.frombuffer(f.read(num_trans*3*4), dtype=np.int16)     
        trans_array = trans_array.reshape((-1,3))
        ...
        while d:
            ...
            if torch.sum(mask_ad)>1:
                ...
                def low_freq2attri(low_freq, w):
                    ...
                    return torch.tensor(yuv), torch.tensor(rgb)
                
                
                low_freq = res['low_freq'][1:][mask_ad]       
                low_freq[low_freq<=0]=0.1   
                res['low_freq'][1:][mask_ad] = low_freq
                res['cyuv'][1:][mask_ad], res['crgb'][1:][mask_ad] = \
                    low_freq2attri(low_freq, w_ad[1:][mask_ad])
                ...
                res['iyuv'][d], res['irgb'][d] = low_freq2attri(iCT_low_d, iW_ad[d])
                ...
                if min_v == max_v:
                    ...
                    context_inter_channel_null = torch.zeros((context_initial_coding.shape[0],1,8)).cuda()
                    
                    
                    context = torch.cat((context_initial_coding[:,None].repeat(1,1,1), 
                                         context_inter_channel_null,
                                         ), -1)    
                    ...
                    query_points_featy_temp = torch.tensor(input_y)                                    
                    high_freq_nodes_y_sp = utils.get_sp_tensor_feat(high_freq_nodes_sp.C, 
                                                                 query_points_featy_temp.float())                       
                    context_y = inter_channel_module.extract_channel(torch.tensor(input_y).cuda().float(), 
                                                         high_freq_nodes_y_sp.cuda(), 
                                                         high_freq_nodes_sp.cuda(), 
                                                         depth_idx=d, 
                    ...
                    # third channel
                    channel_idx = 2
                    input_yu = np.concatenate((input_y, CT_q_dec),-1)
                    
                    query_points_featyu_temp = torch.tensor(input_yu)                                    
                    high_freq_nodes_yu_sp = utils.get_sp_tensor_feat(high_freq_nodes_sp.C, 
                                                                 query_points_featyu_temp.float())                       
                    context_yu = inter_channel_module.extract_channel(torch.tensor(input_yu).cuda().float(), 
                                                         high_freq_nodes_yu_sp.cuda(), 
                                                         high_freq_nodes_sp.cuda(), 
                                                         depth_idx=d, 
                                                         channel_idx=channel_idx)      
                ...
                CT[N_idx_array.astype(int)] = np.concatenate((input_yu, CT_q_dec),-1)*Qstep
